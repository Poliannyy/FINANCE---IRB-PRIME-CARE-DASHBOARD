...
export default function ExameCard({ exame }: ExameCardProps) {
  ...
  const color = categoryColors[exame.categoria] ?? "#6b7280";

  const hasPrimePrices =
    exame.preco_prime_plus != null ||
    exame.preco_prime_essential != null ||
    exame.preco_prime_elite != null;
    ...
    <div style={{
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 3 }}>
            ...
            <span style={{ fontSize: 12, color, fontWeight: 500 }}>{exame.categoria}</span>
          </div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 700, color: "var(--foreground)" }}>
            {exame.preco_particular.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
          </p>
        </div>
        {hasPrimePrices && (
      ...
      {expanded && hasPrimePrices && (
        <div style={{
          ...
          <PriceTag label="Prime Plus" value={exame.preco_prime_plus} />
          <PriceTag label="Prime Essential" value={exame.preco_prime_essential} />
          <PriceTag label="Prime Elite" value={exame.preco_prime_elite} />
        </div>
      )}
    </div>
  );
