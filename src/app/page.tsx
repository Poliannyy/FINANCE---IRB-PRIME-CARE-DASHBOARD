...
const examsData = [
  { id:"1", name:"Hemograma Completo", category:"Laboratorial", price_particular:85 },
  { id:"2", name:"Glicemia em Jejum", category:"Laboratorial", price_particular:35 },
  { id:"3", name:"Ultrassom Abdominal", category:"Imagem", price_particular:280 },
  { id:"4", name:"Eletrocardiograma", category:"Cardiologia", price_particular:150 },
  { id:"5", name:"Raio-X Tórax", category:"Imagem", price_particular:120 },
];

const campaignsData = [
  { id:"1", name:"Check-up de Inverno", description:"Pacote completo de exames laboratoriais com desconto especial na temporada de inverno", discount_percent:25 },
  { id:"2", name:"Campanha Coração Saudável", description:"Exames cardiológicos com desconto para prevenção cardiovascular", discount_percent:20 },
  { id:"3", name:"Promoção Imagem Digital", description:"Exames de imagem com tecnologia de ponta", discount_percent:15 },
];
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:24, boxShadow:"var(--shadow-sm)" }}>
          ...
          {examsData.map((exam, i) => (
            <div key={exam.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 0", borderBottom: i < examsData.length-1 ? "1px solid var(--border)" : "none" }}>
              <div>
                <div style={{ fontSize:13, fontWeight:500 }}>{exam.name}</div>
                <div style={{ fontSize:11, color:"var(--muted)", marginTop:2 }}>{exam.category}</div>
              </div>
              <span style={{ fontSize:13, fontWeight:600, color:"var(--primary)" }}>R$ {exam.price_particular.toFixed(2)}</span>
            </div>
          ))}
        </div>
        <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:"var(--radius-lg)", padding:24, boxShadow:"var(--shadow-sm)" }}>
          ...
          {campaignsData.map(c => (
            <div key={c.id} style={{ padding:14, borderRadius:"var(--radius-sm)", background:"var(--muted-bg)", border:"1px solid var(--border)", marginBottom:10 }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <span style={{ padding:"3px 10px", borderRadius:20, background:"var(--accent-light)", color:"var(--accent-foreground)", fontSize:11, fontWeight:600 }}>{c.discount_percent}% OFF</span>
              </div>
              <p style={{ fontSize:11, color:"var(--muted)", marginTop:4 }}>{c.description}</p>
            </div>
          ))}
        </div>
      </div>
